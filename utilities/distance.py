...
import numpy as np
import utilities.chordUtil as chordUtil
import utilities.C2V
...
def distance_tonnetz(Chord1,
    ...
    if root_1 == root_2 and type_1 == type_2:
        ...
        a,type_1 = utilities.C2V.parse_mir_label(
                utilities.chordUtil.reduChord(Chord1, 'a0'))
        a,type_2 = utilities.C2V.parse_mir_label(
                utilities.chordUtil.reduChord(Chord2, 'a0'))
...
def distance_euclid(A_1,
    ...
    '''
    #initialisation
    A_1_red = utilities.chordUtil.reduChord(A_1, 'reduceWOmodif')
    A_2_red = utilities.chordUtil.reduChord(A_2, 'reduceWOmodif')
    Accord_1_eucl = utilities.C2V.mir_label_to_bin_chroma_vec(A_1_red)
    Accord_2_eucl = utilities.C2V.mir_label_to_bin_chroma_vec(A_2_red)
...
def computeEuclideanPitchVectMat(dictChord):
    matrix = []
    for i in (dictChord):
        matrix.append(utilities.C2V.mir_label_to_bin_chroma_vec(i))
    return matrix
...
def save_tonnetz_matrix():
    ...
    inputs0, alphabet, mapping0 = utilities.chordUtil.reduSeq(inputs, 'a0')
...
def representation(chord, rho):
    ...
    if chord == '':
        ...
        for i in interval[ChordType]:
            if i > 0:
                if (i + number) < 12:
                    #print('Here')
                    for j in range(i):
                        newroot = chordUtil.tr[newroot]
